...
for evt in data.get_events():
    ...
    if not ((zenith > 113*units.deg) and (zenith < 117*units.deg) and (chrenkov_angle > 56*units.deg) and (chrenkov_angle < 57*units.deg)):
        continue
    if not ((azimuth > 55*units.deg) and (azimuth < 65*units.deg)):
        continue
    if not (weight > 0.5):
        continue
    ...
    for i in range(8):
        ...
        ax.set_xlim(time[max_index-100], time[max_index+100])
    ...
    lpda_tplt = stn.get_channel(1).get_trace()
